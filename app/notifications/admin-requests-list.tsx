...
export default function AdminRequestsList() {
  ...
  const fetchConnections = useCallback(
    async (email: string): Promise<ConnectionRow[]> => {
      const { data, error } = await supabase
        .from("connections")
        .select("*")
      ...
      if (emails.length > 0) {
        ...
        if (userErr) {
          ...
          namesByEmail = Object.fromEntries(
            (userRows ?? []).map((u: any) => [u.email, u.name]),
          );
        }
      }
      ...
      return rows.map((r) => ({
        ...r,
        employee_name: namesByEmail[r.employee_email] ?? r.employee_email,
      }));
    },
    [],
  );
  ...
  const fetchExtensionRequests = useCallback(
    async (wsId: string): Promise<ExtensionRequestRow[]> => {
      const { data, error } = await supabase
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [conns, exts] = await Promise.all([
    ...
    setConnections(
      conns.filter((c) => !cleared.connections.includes(c.employee_email)),
    );
    setExtensionRequests(
      exts.filter((r) => !cleared.extensions.includes(r.id)),
    );
    setLoading(false);
...
const decideConnection = async (employeeEmail: string, decision: "accepted" | "rejected") => {
  ...
  const clearAllConnections = async () => {
    ...
    const decidedConnectionKeys = connections
      .filter((c) => isDecided(c.status))
      .map((c) => c.employee_email);
    ...
    const updated = {
      ...
      connections: Array.from(
        new Set([...existing.connections, ...decidedConnectionKeys]),
      ),
    };
    await AsyncStorage.setItem(
      clearedKey(workspaceId),
  ...
  const clearAllExtensions = async () => {
    ...
    const decidedExtensionIds = extensionRequests
    ...
    const updated = {
      ...
      extensions: Array.from(
        new Set([...existing.extensions, ...decidedExtensionIds]),
      ),
    };
    await AsyncStorage.setItem(
      clearedKey(workspaceId),
  ...
  const hasDecidedExtensions = extensionRequests.some((r) =>
    isDecided(r.status),
  );
