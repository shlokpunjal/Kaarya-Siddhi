...
export default function EmployeeTasks() {
    ...
    const fetchEmployeeTasks = useCallback(async (isMounted: () => boolean = () => true) => {
        if (!employeeEmail) return;

        const { data: employeeUser, error: userLookupError } = await supabase
            .from('users')
            .select('id')
        ...
        if (userLookupError || !employeeUser) {
            console.error('Could not resolve user id for email:', employeeEmail);
            return;
        }
            ...
            >
                ...
                <View style={{
                    ...
                    <View style={{ marginLeft: wp(2) }}>
                        <Text style={{ ...typography.subheading, color: colors.text.secondary }}>
                            Tasks assigned to
                        </Text>
