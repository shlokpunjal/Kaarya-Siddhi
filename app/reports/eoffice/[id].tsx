...
import EofficeDetailSkeleton from "../../../components/skeletonScreens/EOffice/EofficeDetailSkeleton";
...
export default function EofficeDetail() {
  ...
  const canEdit =
    !!currentUser &&
    !!file &&
    (currentUser.role === "admin" || currentUser.id === file.created_by);
  ...
  const confirmToggleCompleted = async () => {
    if (!file || pendingValue === null) return;
    setConfirmVisible(false);
    try {
      setSaving(true);
      const updated = await updateEofficeFile(file.id, {
        completed: pendingValue,
      });
      setFile(updated);
    } catch (err) {
    ...
    >
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
        ...
        >
          ...
          {file.completed
            ? "Completed"
            : `Pending ${daysPending(file.pending_since)} days`}
        </Text>
        ...
        <Text
          style={[
            typography.heading3,
            { color: colors.text.secondary, marginTop: 18, marginBottom: 8 },
          ]}
        >
          Pending Office
        </Text>
        ...
        {file.completed && (
          <View
            style={[
              styles.noticeBox,
              {
                backgroundColor: colors.status.completed + "22",
                marginTop: 16,
              },
            ]}
          >
      ...
      <AlertModal
        ...
        cancelText="Cancel"
        onConfirm={confirmToggleCompleted}
