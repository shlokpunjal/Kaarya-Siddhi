...
export default function GenPdf() {
  ...
  const buildQuery = () => {
    ...
    if (startDate) params.append("start_date", toLocalDateString(startDate));
    if (endDate) params.append("end_date", toLocalDateString(endDate));
    if (selectedValue) {
  ...
  const handleRangePresetChange = (preset: RangePreset) => {
    ...
    if (computed) {
      ...
      // Custom — clear so the user has to explicitly pick both
      setStartDate(null);
      setEndDate(null);
    }
  };
  ...
  const onChangeDate = (event: any, selected?: Date) => {
    ...
    // Ensure custom range preset is active if user changes date
