...
import AnimatedTrainScene from "./AnimatedTrainScene";
import DashboardSkeleton from "./DashboardSkeleton";
import LoginChoiceSkeleton from "../components/DashboardSkeleton";
import { useNetworkStatus } from "../hooks/useNetworkStatus";
import { getToken } from "../lib/secureSession";
import { typography } from "../theme/theme";

const MIN_CHECK_DURATION_MS = 700; // avoid an instant flash on fast checks

type SkeletonVariant = "dashboard" | "login" | null;
...
export default function OfflineScreen({
    ...
    const [skeletonVariant, setSkeletonVariant] = useState<SkeletonVariant>(null);
    const spin = useRef(new Animated.Value(0)).current;
    ...
    const handleRetry = async () => {
        ...
        // Session check is local (SecureStore) and near-instant — decide
        // which skeleton shape to show before/independent of the network probe.
        const token = await getToken();
        setSkeletonVariant(token ? "dashboard" : "login");
        ...
        return (
            <View style={styles.container}>
                <View style={styles.checkingRow}>
                    ...
                    <Text style={[styles.checkingText, typography.heading3]}>Checking connection…</Text>
                </View>
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            <AnimatedTrainScene />
            <Text style={[styles.title, typography.subheading]}>No Internet Connection</Text>
            <Text style={[styles.subtitle, typography.body]}>
                Kaarya Siddhi will reconnect automatically once you're back online.
            </Text>
            <Pressable style={styles.retryButton} onPress={handleRetry}>
