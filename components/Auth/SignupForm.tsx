...
const ROLE_CONFIG = {
  admin: {
    headerTitle: "Admin Signup",
    cardTitle: "Create Admin Account",
    loginRoute: "/(auth)/AdminLogin",
  },
  employee: {
    headerTitle: "Employee Signup",
    cardTitle: "Create Employee Account",
    loginRoute: "/(auth)/EmployeeLogin",
  },
} as const;
...
const SignupForm = ({ role }: SignupFormProps) => {
  ...
  }, [name, role]); // ensures that name is unique
  ...
  async function createAccount() {
    ...
    try {
      ...
      const signupResponse = await fetch(`${API_BASE_URL}/signup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, department, email, phone, role }),
      });
      ...
      if (!signupResponse.ok) {
        ...
        if (detail.includes("phone")) {
          setErrors((prev) => ({ ...prev, phone: signupData.detail }));
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <BackButton />
        <Text style={[styles.headerText, typography.heading]}>{config.headerTitle}</Text>
      </View>
        ...
        >
          <View style={styles.mainStyle}>
            ...
            <View style={[styles.card, cardError ? styles.cardError : null]}>
              <Text style={[styles.title, typography.subheading]}>{config.cardTitle}</Text>
            ...
            <View style={styles.login}>
              ...
              <TouchableOpacity onPress={() => router.replace(config.loginRoute)}>
...
const styles = StyleSheet.create({
  ...
  mainStyle: { alignItems: "center", width: "100%" },
  header: {
    width: "100%",
  ...
  buttonDisabled: { backgroundColor: "#6B7280", shadowOpacity: 0, elevation: 0 },
  buttonText: {
    color: "white",
    fontSize: 16,
    fontFamily: "Poppins_400Regular",
  ...
  login: { alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 3 },
  loginT: {
    color: PRIMARY,
    fontFamily: "Poppins_600SemiBold",
    marginTop: 27.4,
