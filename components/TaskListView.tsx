...
import { View, Text, StyleSheet, ScrollView, Pressable, Modal, TextInput, Platform, RefreshControl } from "react-native";
...
import AdminTasksSkeleton from "./AdminTasksSkeleton";
import {
  FilterType, ManagedEmployee, STATUS_RANK, STATUS_LABELS, PRIORITY_RANK,
} from "../utils/taskList";

interface TaskListViewProps {
  taskDetailRoute: string;
  searchPlaceholder: string;
  showEmployeeColumn: boolean; // admin sees who a task is assigned to; employee doesn't need to
  // Returns freshly fetched tasks (+ employees, only meaningful when showEmployeeColumn is true).
  // Owns its own auth/redirect guard if it needs one.
  fetchTasks: () => Promise<{ tasks: Task[]; employees: ManagedEmployee[] }>;
}

export default function TaskListView({ taskDetailRoute, searchPlaceholder, showEmployeeColumn, fetchTasks }: TaskListViewProps) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const { tasks: t, employees: e } = await fetchTasks();
      setTasks(t);
      setEmployees(e);
    } catch (err) {
  ...
  const openModal = () => { setDraftType(appliedType); setDraftValue(appliedValue); setModalVisible(true); };
  const selectDraft = (type: FilterType, value: string | null) => { setDraftType(type); setDraftValue(value); };
  const applyFilter = () => { setAppliedType(draftType); setAppliedValue(draftValue); setModalVisible(false); };
  ...
  const getVisibleTasks = () => {
    ...
    if (appliedType === "label" && appliedValue) list = list.filter((t) => (t.label ?? "").toLowerCase() === appliedValue.toLowerCase());
    ...
    if (appliedType === "priorityHighLow") list.sort((a, b) => PRIORITY_RANK[b.priority] - PRIORITY_RANK[a.priority]);
    if (appliedType === "priorityLowHigh") list.sort((a, b) => PRIORITY_RANK[a.priority] - PRIORITY_RANK[b.priority]);
    ...
    if (query.length > 0) {
      list = list.filter((t) =>
        ...
        (showEmployeeColumn && employeeDisplayName(t.assignedTo).toLowerCase().includes(query))
      );
    }
    return list;
  };
  ...
  const Chip = ({ label, type, value }: { label: string; type: FilterType; value: string | null }) => {
    ...
    return (
      <Pressable
        ...
        style={[styles.chip, { backgroundColor: selected ? colors.brand.accent : colors.base.surfaceL2, borderColor: selected ? colors.brand.accent : colors.base.border }]}
      >
        <Text style={[typography.label, { color: selected ? "#FFFFFF" : colors.text.primary }]}>{label}</Text>
      </Pressable>
    );
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.base.background }]}>
      <View style={styles.headerRow}>
        ...
        <Pressable style={[styles.filterButton, { backgroundColor: colors.base.surfaceL1, borderColor: colors.base.border }]} onPress={openModal}>
          <Text style={[typography.heading3, { color: colors.brand.accent }]}>Filter</Text>
        </Pressable>
      </View>

      <View style={styles.searchRow}>
        <View style={[styles.searchBar, { backgroundColor: colors.base.surfaceL1, borderColor: searchFocused ? colors.brand.accent : colors.base.border, borderWidth: searchFocused ? 1.5 : 1 }, searchFocused && styles.searchBarFocused]}>
          <Ionicons name="search" size={18} color={searchFocused ? colors.brand.accent : colors.text.secondary} style={styles.searchIcon} />
          <TextInput
            value={searchQuery}
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.brand.accent} colors={[colors.brand.accent]} />}
      >
            ...
            >
              <View style={styles.taskCardHeader}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: colors.status[task.status] }]}>
                  <Text style={[typography.label, styles.statusBadgeText]}>{STATUS_LABELS[task.status]}</Text>
                </View>
              </View>
              <Text style={[typography.label, { color: colors.text.secondary, marginTop: 6 }]}>
                {showEmployeeColumn ? `${employeeDisplayName(task.assignedTo)} · ` : ""}
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setModalVisible(false)}>
          <Pressable style={[styles.modalCard, { backgroundColor: colors.base.surfaceL1 }]} onPress={() => {}}>
            <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollArea}>
              <Text style={[typography.subheading, { color: colors.text.primary, marginBottom: 16 }]}>Filter Tasks</Text>
              ...
              <Text style={[typography.label, { color: colors.text.secondary, marginTop: 14, marginBottom: 6 }]}>By Status</Text>
              <View style={styles.chipRow}>
              ...
              <Text style={[typography.label, { color: colors.text.secondary, marginTop: 14, marginBottom: 6 }]}>By Priority</Text>
              <View style={styles.chipRow}>
                {(["low", "medium", "high"] as TaskPriority[]).map((priority) => (
                  <Chip key={priority} label={priority.charAt(0).toUpperCase() + priority.slice(1)} type="priority" value={priority} />
                ))}
              </View>

              <Text style={[typography.label, { color: colors.text.secondary, marginTop: 14, marginBottom: 6 }]}>By Label</Text>
              <View style={styles.chipRow}>
                {uniqueLabels.map((label) => <Chip key={label} label={label} type="label" value={label} />)}
              </View>

              {showEmployeeColumn && (
                <>
                  <Text style={[typography.label, { color: colors.text.secondary, marginTop: 14, marginBottom: 6 }]}>By Employee</Text>
                  <View style={styles.chipRow}>
                    {employees.map((emp) => <Chip key={emp.id} label={emp.name || emp.email} type="employee" value={emp.id} />)}
                  </View>
                </>
              )}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  searchRow: { paddingHorizontal: wp(5.3), paddingBottom: 14 },
  searchBar: {
    flexDirection: "row", alignItems: "center", borderRadius: 16, paddingHorizontal: 14, paddingVertical: 11,
    ...Platform.select({ ios: { shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6 }, android: { elevation: 1 } }),
  },
  searchBarFocused: { ...Platform.select({ ios: { shadowOpacity: 0.12, shadowRadius: 8 }, android: { elevation: 3 } }) },
