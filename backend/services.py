...
#     # Tables with FK constraints on users.id (delete_rule = NO ACTION, so these
#     # must be cleaned up manually or the users.delete() below will fail)
...
#     # tasks.assigned_to / created_by are untyped text (no FK), and in practice
#     # hold a mix of emails, names, and occasional UUIDs -- so match on both.
#     supabase.table("tasks").delete().eq("assigned_to", email).execute()
#     supabase.table("tasks").delete().eq("assigned_to", user_id).execute()
#     supabase.table("tasks").delete().eq("created_by", email).execute()
...
#         supabase.table("workspaces").delete().eq("owner_email", email).execute()
